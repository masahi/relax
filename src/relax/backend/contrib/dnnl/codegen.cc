...
class DNNLJSONSerializer : public JSONSerializer {
 public:
  DNNLJSONSerializer(const std::string& symbol, const Map<Var, Expr>& bindings, Map<Constant, String> constant_names)
    : JSONSerializer(symbol, constant_names), bindings_(bindings) {}
...
Array<runtime::Module> DNNLCompiler(Array<Function> functions, Map<String, ObjectRef> /*unused*/, Map<Constant, String> constant_names) {
  Array<runtime::Module> compiled_functions;

  for (const auto& func : functions) {
    auto func_name = GetExtSymbol(func);
    DNNLJSONSerializer serializer(func_name, AnalyzeVar2Value(func), constant_names);
